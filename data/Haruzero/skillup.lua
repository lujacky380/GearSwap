...
function status_change(new,old)
    ...
    if new=='Idle' then
        if skilluprun then
            send_command('wait 1.0;input /ma "'..gs_skill.skillup_spells[gs_skill.skillup_count]..'" <p1>')
        end
...
function filtered_action(spell)
    ...
    if gs_skill.bluspellulid[spell.en] then
        if windower.ffxi.get_ability_recasts()[81] and windower.ffxi.get_ability_recasts()[81] == 0 then
            cancel_spell()
            send_command('input /ja "'..res.job_abilities[298][gearswap.language]..'" <p1>')
        elseif windower.ffxi.get_ability_recasts()[254] and windower.ffxi.get_ability_recasts()[254] == 0 then
            cancel_spell()
            send_command('input /ja "'..res.job_abilities[338][gearswap.language]..'" <p1>')
        else
            cancel_spell()
            gs_skill.skillup_count = (gs_skill.skillup_count % #gs_skill.skillup_spells) + 1
            send_command('input /ma "'..gs_skill.skillup_spells[gs_skill.skillup_count]..'" <p1>')
        end
        return
    elseif spell.skill == "Ninjutsu" then
        ...
        cancel_spell()
        if tool then
            send_command('input /item "'..tool..'" <p1>')
        end
        return
    elseif S{"Avatar's Favor","Elemental Siphon"}:contains(spell.en) then
        cancel_spell()
        send_command('input /ja "'..res.job_abilities[90][gearswap.language]..'" <p1>')
    end
...
function precast(spell)
    if pet.isvalid and S{"Release","Full Circle"}:contains(spell.en) then
        if spell.en == "Release" then
            ...
            if (recast > 0) then
                cancel_spell()
                send_command('wait '..tostring(recast+0.5)..';input /ja "'..res.job_abilities[90][gearswap.language]..'" <p1>')
            end
            return
        else
            return
    ...
    if gs_skillup.use_trust and party.count == 1 and spell_usable(res.spells[931]) then
        if spell.en ~= "Moogle" then
            cancel_spell()
            send_command('input /ma "'..res.spells[931][gearswap.language]..'" <p1>')
            return
        end
    end
    if gs_skillup.use_item and spell.type ~= "Item" and not buffactive[251] then
        for i,v in ipairs(gs_skill.skill_up_item) do
            if player.inventory[res.items[v][gearswap.language]] then
                cancel_spell()
                send_command('input /item "'..res.items[v][gearswap.language]..'" <p1>')
                return
            end
        end
    elseif gs_skillup.use_geo then
        if player.main_job == "GEO" and spell_usable(res.spells[800]) and not pet.isvalid then
            if spell.en ~= "Geo-Refresh" then
                cancel_spell()
                send_command('input /ma "'..res.spells[800][gearswap.language]..'" <p1>')
                return
            end
            return
        elseif player.sub_job == "GEO" and spell_usable(res.spells[770]) and buffactive[541] ~= (gs_skillup.use_trust and 2 or 1) then
            if spell.en ~= "Indi-Refresh" then
                cancel_spell()
                send_command('input /ma "'..res.spells[770][gearswap.language]..'" <p1>')
                return
            end
        end
    end
...
function aftercast(spell)
    if pet.isvalid and check_skill_cap() then
        if pet.name == "Luopan" then
            send_command('wait 1.0;input /ja "'..res.job_abilities[345][gearswap.language]..'" <p1>')
        else
            send_command('wait 1.0;input /ja "'..res.job_abilities[90][gearswap.language]..'" <p1>')
        end
        return
    end
    ...
    elseif spell.interrupted then
        if spell.en == "Release" then
            send_command('wait 0.5;input /ja "'..res.job_abilities[90][gearswap.language]..'" <p1>')
            return
        else
            send_command('wait 3.0;input /ma "'..spell.name..'" <p1>')
            return
        end
    elseif spell.type == "SummonerPact" then
        ...
        if spell.en:contains('Spirit') and (spell_element == world.weather_element or spell_element == world.day_element) then
            send_command('wait 4.0;input /ja "'..res.job_abilities[232][gearswap.language]..'" <p1>')
        elseif not spell.en:contains('Spirit') then
            send_command('wait 4.0;input /ja "'..res.job_abilities[250][gearswap.language]..'" <p1>')
        else
            send_command('wait 3.0;input /ja "'..res.job_abilities[90][gearswap.language]..'" <p1>')
        end
    ...
    elseif spell.en == "Elemental Siphon" then
        ...
        send_command('wait 3.0;input /ma "'..gs_skill.skillup_spells[gs_skill.skillup_count]..'" <p1>')
    end
end
...
function spell_valid(tab)
    if (tab.levels[player.main_job_id] and tab.levels[player.main_job_id] <= player.main_job_level or tab.levels[player.sub_job_id] and tab.levels[player.sub_job_id] <= player.main_job_level) and tab.targets:contains('Self') and
        not tab.en:wmatch('Teleport-*|Warp*|Tractor*|Retrace|Escape|Geo-*|Sacrifice|Odin|Alexander|Recall-*|Phalanx|Reprisal|Crusade|Protect*|Shell|Shell II|Shell III') then
        return true
    end
end
